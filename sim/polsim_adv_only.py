...
def rhs_adv(U: torch.Tensor, Vx: float | torch.Tensor, Vy: float | torch.Tensor, k: torch.Tensor, S: torch.Tensor, dx: float, dy: float) -> torch.Tensor:
    """
    RHS of advection-source PDE:
      dU/dt = - (Vx dU/dx + Vy dU/dy) + S
    """
    adv = advection_upwind(U, Vx, Vy, dx, dy)
    return -adv + S
...
def rollout_pollution(
    ...
    def _simulate() -> Dict[str, torch.Tensor]:
        ...
        for n in range(steps):
            ...
            f0 = rhs_adv(U, Vx_t, Vy_t, k, S_total, grid.dx, grid.dy)
            f0 = torch.nan_to_num(f0, nan=0.0, posinf=0.0, neginf=0.0)

            U_star = U + float(dt) * f0
            f1 = rhs_adv(U_star, Vx_t, Vy_t, k, S_total, grid.dx, grid.dy)
