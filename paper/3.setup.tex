...
dimension \(n\).  Over a fixed observation window, source activity is represented
by a nonnegative vector
\begin{equation}
\boldsymbol\theta\in \mathbb R_+^K,
\label{eq:source_activity}
\end{equation}
so that emissions are proportional to \(S\boldsymbol\theta\).  Time-varying
activity can be handled by replacing \(\boldsymbol\theta\) with a sequence or a
low-dimensional temporal basis; the constant-window model is the basic object
analyzed here.
...
\label{eq:lag_set}
\end{equation}
The source-to-sensor response at time \(t\) is
\begin{equation}
H_t^{\mathrm{lag}}
= \sum_{\ell\in\mathcal L_t}
O G_{t,t-\ell}^{\partial}(w_{t-\ell:t}) S
\in \mathbb R^{m\times K}.
\label{eq:time_response}
\end{equation}
Stacking over the finite horizon gives
\begin{equation}
H_{1:T}^{\mathrm{lag}}
=
\begin{bmatrix}
...
and write
\begin{equation}
Y = H_{1:T}^{\mathrm{lag}}\boldsymbol\theta + Q\boldsymbol\gamma + E.
...
\(\widetilde H\), not from the raw inventory matrix or the unprojected response.

\paragraph{Projected source fingerprints.}
Writing
\begin{equation}
H_{1:T}^{\mathrm{lag}}
=[\mathbf h_1^{\mathrm{lag}}\;\cdots\;\mathbf h_K^{\mathrm{lag}}],
\qquad
\widetilde H=[\widetilde{\mathbf h}_1\;\cdots\;\widetilde{\mathbf h}_K],
\label{eq:source_fingerprints}
\end{equation}
we have
\(\widetilde{\mathbf h}_k=P_Q^\perp\mathbf h_k^{\mathrm{lag}}\).  These
projected fingerprints encode the observable effect of each source group after
wind transport, lag, sparse sensing, and background correction.  Source
apportionment is well posed only to the extent that these fingerprints are
