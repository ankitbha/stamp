...
wind-conditioned lagged operator, defining the observation model, inventory
...
distinguishability analyzed here; signed temporal harmonics belong to the separate
background basis \(Q\).  We vectorize \(C\) in source-major, basis-minor order as
...
\(\widehat{\mathbf w}_t(\mathbf x_g)\); a learned imputer was trained but not
adopted (Section~\ref{subsec:results_wind_imputation}), and the same interface
...
coordinates; effective rank capped at eight, the rank-four member for New Delhi),
...
\end{equation}
with an alignment mismatch treated as an error rather than a silent reordering;
