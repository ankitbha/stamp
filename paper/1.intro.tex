...
lower-dimensional source activity vector and make the problem interpretable:
the output is an estimate of the activity or contribution of each candidate
source group.  However, the inventory restriction alone does not make the
problem identifiable.  Source groups can still be weakly visible, absorbed by
background components, or mutually coherent after transport to the sensor
network.
...
and lag window, each source group induces a finite-horizon sensor-space
fingerprint.  Stacking these fingerprints gives a lagged response matrix
\(H_{1:T}^{\mathrm{lag}}\).  Real observations also contain background and
...
background basis.  The rank, singular values, source visibility, background
absorption, and pairwise coherence of \(\widetilde H\) determine which source
contributions can be separated and which should be reported only after merging
or qualification.
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=0pt]
    ...
    lagged wind-conditioned inverse problem over known or proxy source groups.
    \item We identify the projected lagged response matrix
    \(\widetilde H=P_Q^\perp H_{1:T}^{\mathrm{lag}}\) as the central object
    governing source separability after transport, sparse observation, lag, and
    ...
    source visibility, background absorption, and pairwise source-fingerprint
    coherence.
    \item We propose an identifiability-aware apportionment procedure that fits
    nonnegative source contributions, reports uncertainty, flags weakly visible
    sources, and merges source groups whose fingerprints are indistinguishable
    under the current sensing system.
    ...
    merging are predicted by the geometry of \(\widetilde H\) under varying
...
source fingerprints are visible and separable in the observed sensor space.
