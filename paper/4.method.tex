...
declared initial-condition baseline.  For observed New Delhi runs, the initial
pollution field is estimated from the first available regulatory observations
using spatial kriging on the response grid, and its zero-source transported
trajectory is subtracted from the sensor observations before fitting source
activities.  Source coefficients are therefore fitted to the residual
sensor-time signal after removing the contribution of the initialized
background field and the low-rank temporal background basis.  Controlled
operator-matched experiments may instead use a zero initial field when the
synthetic data are generated from the same zero-initial-state response.
