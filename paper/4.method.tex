...
Given the estimated wind field and source inventory, the lagged source response
is constructed as in Section~\ref{sec:problem_setup}:
\begin{equation}
\begin{aligned}
&H_t^{\mathrm{lag}}
=\sum_{\ell\in\mathcal L_t}
O G_{t,t-\ell}^{\partial}(\widehat{\mathbf w}_{t-\ell:t};\psi)S, \\
&H_{1:T}^{\mathrm{lag}}=[H_1^{\mathrm{lag}};\ldots;H_T^{\mathrm{lag}}].
\end{aligned}
...
\end{equation}
Source activities are then estimated by nonnegative regularized least squares,
\begin{equation}
\widehat{\boldsymbol\theta}
=\arg\min_{\boldsymbol\theta\ge 0}
...
\begin{equation}
R(\boldsymbol\theta)=\|\boldsymbol\theta-\boldsymbol\theta_0\|_2^2.
...
(\widehat{\boldsymbol\theta},\widehat{\boldsymbol\gamma})
=\arg\min_{\boldsymbol\theta\ge 0,\boldsymbol\gamma}
\|Y-H_{1:T}^{\mathrm{lag}}\boldsymbol\theta-Q\boldsymbol\gamma\|_2^2
+\lambda R(\boldsymbol\theta).
\label{eq:joint_fit}
\end{equation}
The projected formulation is used for identifiability because it exposes the
part of each source fingerprint that remains after background correction.
...
\begin{align}
\mathcal L_{\mathrm{refine}}
=&\|Y-H_{1:T}^{\mathrm{lag}}(\phi,\psi)\boldsymbol\theta-Q\boldsymbol\gamma\|_2^2
+\lambda_\theta R(\boldsymbol\theta) \nonumber\\
