...
\end{equation}
The observed station vectors and masks are supplied to a FieldFormer wind
imputer \citep{bhardwaj2025fieldformer}.  FieldFormer is trained as a
coordinate-query model: given sparse station observations
...
we query the trained model on every response-grid cell \(\mathbf x_g\) and hour
\(t\), producing
\begin{equation}
