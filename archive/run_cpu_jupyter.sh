...
singularity exec --nv \
	...
	source /ext3/env.sh;
	port=8881;
	/usr/bin/ssh -N -f -R ${port}:localhost:${port} torch-login-3;
	/usr/bin/ssh -N -f -R ${port}:localhost:${port} torch-login-2;
	/usr/bin/ssh -N -f -R ${port}:localhost:${port} torch-login-1;
